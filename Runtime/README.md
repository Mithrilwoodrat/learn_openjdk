...
在 fields 字段表后面是方法表。和字段表一样，开头是方法表个数
 `methods_count` 然后是 method_info 数组,结构如下:
...
可以看出结构和 `field_info` 基本一致。

#### access_flags

方法表的 access_flags 和字段表也基本相同，少了 `volatile` `transient` `enum`，因为这几个关键字不能修饰方法，然后多了 `synchronized` `native` `strictfp` `abstract` `VARARGS`等关键字。
...
#### name_index 方法名索引 和 descriptor_index 描述符索引

 方法区 method_info 中 name_index 的值同样为常量池中的索引，对应的值依然是 `CONSTANT_Utf8_info` 。 `TestClass` 中第一个方法名索引为 20,对应 `<init>`

`descriptor_index` 同上，`TestClass` 第一个方法的描述符索引为 21，对应 `()V`，即参数为空，返回值为 void。

方法描述符的格式参考[文档 Method Descriptors](https://docs.oracle.com/javase/specs/jvms/se7/html/jvms-4.html#jvms-4.3.3)
